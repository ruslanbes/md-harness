...
Ad-hoc changelog updates without a version tag are fine anytime; this runbook is for **promoting `[Unreleased]` → `[X.Y.Z]`** and cleaning finished task artifacts (backlog rows + `tasks/` detail files).
